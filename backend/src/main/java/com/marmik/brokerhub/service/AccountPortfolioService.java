...
public class AccountPortfolioService {
    ...
    /**
     * Aggregate positions for the account (similar to holdings flow).
     */
    public List<AggregatedPosition> aggregatePositionsForAccount(UUID accountId, UUID callerUserId) {
        // 1) fetch members for account
        List<AccountMember> members = memberRepo.findByAccountId(accountId);
        if (members.isEmpty()) {
            return Collections.emptyList();
        ...
        // 3) For each credential, perform decrypt + broker call concurrently
        List<CompletableFuture<List<PositionItem>>> futures = new ArrayList<>();
        for (BrokerCredential cred : allCreds) {
            CompletableFuture<List<PositionItem>> f = CompletableFuture.supplyAsync(() -> {
                byte[] plain = null;
                try {
                    // decrypt token (validates caller membership internally)
                    UUID credId = cred.getCredentialId(); // <--- use actual getter on your entity
                    plain = credentialService.decryptCredentialToken(callerUserId, credId);
                    if (plain == null || plain.length == 0) {
                        return Collections.<PositionItem>emptyList();
                    }
        ...
        // wait for completions (with timeout safety)
        List<PositionItem> combined = new ArrayList<>();
        try {
            CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).get(30, TimeUnit.SECONDS);
            for (CompletableFuture<List<PositionItem>> cf : futures) {
                try {
                    List<PositionItem> list = cf.getNow(Collections.emptyList());
                    if (list != null && !list.isEmpty())
                        combined.addAll(list);
                } catch (CompletionException ce) {
                    // skip failed task
        ...
        // 5) Convert accumulators to DTOs
        List<AggregatedPosition> result = acc.values().stream()
    ...
    /**
     * Position accumulator (mirrors holdings accumulator but for PositionItem)
     */
    private static class PositionAccumulator {
        ...
        void add(PositionItem p) {
            ...
            totalQty += qty;
            totalValue += qty * avgPrice;
            // sum total pnl per position (realized + unrealized)
            pnlSum += (p.getTotalPnl());

            // prefer non-zero lastPrice: pick most recent non-zero encountered
            if (p.getLastPrice() != 0.0) {
                lastPrice = p.getLastPrice();
                // PositionItem currently doesn't have dayChange fields; left as zero
            }
        }
